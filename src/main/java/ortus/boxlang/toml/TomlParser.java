...
public final class TomlParser {
	...
	public IStruct deserialize( IBoxContext context, String toml, IStruct options ) {
		...
		// Normalize line endings so that TOML parsing produces consistent string values regardless
		// of the line-ending style that the OS or VCS checkout introduced. TOML 1.0.0 §1 permits LF
		// and CRLF document line endings, but tomlj preserves carriage returns inside multiline
		// strings whereas the toml-test conformance fixtures always expect LF.
		if ( toml.indexOf( '\r' ) >= 0 ) {
			toml = toml.replace( "\r\n", "\n" ).replace( '\r', '\n' );
		}
